 ...
 * `changedFiles` が示すのは「打ち切られる前に変更を作っていた」ことであって、
 * **打ち切られた瞬間も進んでいた**ことではない（独立レビュー指摘）。
 * 60 秒で書いてその後ハングした Job も同じ行になる。
 *
 * 「打ち切り時点でも進んでいた」と言えるのは **7 件のうち 1 件だけ**である:
 * `2dc04370` は session transcript の毎分イベント数が 23/12/12/14/**34** で、
 * 最後の 1 分が最多、最終イベントは終了の 15 秒前だった。
 * 残り 6 件については、進捗の timestamp が残っていないので**分からない**
 * （この「分からなさ」自体は `cross-project-state-api` へ記録済み）。
 *
