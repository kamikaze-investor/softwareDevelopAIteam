...
12. [ ] **Meta Reviewer structured-output robustness / false-BLOCKED の解消** — 2026-09-07、PR #98 / #99
      ...
      **2026-09-17 追加実測（PR #234）— 事象1・事象2 の両方が同一 PR で再現し、事象2 は rebase で消えた**:
      - 1回目（base `7bd180a`）: Gemini は ```json フェンス付きで `"status": "changes_requested"` /
        `riskLevel: medium` / `requiresCeoApproval: false` を返したが parser が失敗し **BLOCKED / critical**。
        指摘内容は「follow-up 機能が削除されている」で、これは **#233 の追加を削除と読んだ phantom deletion**
        （PR の head が base より古く、二点間 diff になっていた）。事象2 の実例である
      - 2回目（`cfa3410` へ rebase 後）: phantom deletion の指摘は**消えた**。Gemini の実質判定は
        **`"status": "approved"`**。それでも **BLOCKED / critical** になった

      **【重要な訂正 2026-09-17】本 item の事象1の記述「fenced JSON を返すと parser が失敗する」は誤りである。**
      実測で否定された。`extractJsonCandidates()`（`runner.ts:286`）は既に
      ```` ```json ```` / ```` ``` ```` / 前後に散文がある場合をすべて処理でき、`parseMetaReviewResult()` を
      直接叩いて確認した（raw / ```json / ``` / 散文つき の4形式すべて PARSED、truncated と malformed は
      正しく blocked）。**フェンス正規化は既に実装済みであり、ここを直しても何も変わらない。**

      **実際の機構は応答の切り捨てである。** PR #234 の2回の失敗はいずれも
      **全文字数 859 / 848 文字**で、末尾が `"message": "センサー発火の重複` のように
      **文字列の途中で切れている**（閉じ引用符も閉じ波括弧も閉じフェンスも無い）。
      JSON が不完全なので全候補が parse できず、fail-closed は**正しい挙動**である。

      **未確定（次に測るもの）**: 850 文字前後は `maxOutputTokens: 8192`（`geminiClient.ts:39`。
      gemini-2.5-flash の上限）に遠く及ばない。`gemini-2.5-flash` は thinking model なので、
      thinking token が同じ出力予算を食い潰して可視出力が残らなかった疑いが強いが、**断定しない**。
      現在のコードは `finishReason` と `usageMetadata`（`thoughtsTokenCount`）を捨てているため、
      機構を確定できない。**着手時の最初の一手は推測での修正ではなく、この2つをログへ残すこと。**
      （過去に 4096 → 8192 へ上げた際も同じ「途中で切れる」症状であり、diff が大きいほど再現しやすい
      という既存の観察と整合する。#234 は約 3,100 行、#98 は約 4,300 行）
      - 影響: `mergeStateStatus` が `BLOCKED` になり、**approved な PR が機械的に merge できない**。
        PR #234 はこれで停止し、CEO 判断を仰ぐことになった

      **この item の再評価条件（観測を閉ループにする）**: 着手条件は既に満たされている
      （approved な PR が required check で止まった実例が 1 件ある）。**次に同じ事象が起きたら着手**ではない。
      ただし**着手内容は fenced JSON 正規化ではない**（上記訂正のとおり実装済み）。
      着手順序は (1) `finishReason` / `usageMetadata` をログへ残して機構を確定する、
      (2) 確定した機構に対して最小修正、とする。三点間 diff（事象2）は #234 の rebase で
      回避できたため優先度を下げてよい。`observation-closes-loop` に従い、この観測は
      「様子見」ではなく**次に測る値と測る場所を指定した状態**でここに閉じている。
