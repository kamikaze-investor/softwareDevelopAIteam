...
**背景（2026-07-06の反省）:** Target Project Risk Scanは長期間「観察モード」として動いていたが、結果を一切永続化しておらず、high/medium/lowの件数・誤検知率・有効性を後から評価できない状態だった（コミットcc9c95fで`review_observation.jsonl`として最小永続化を追加し是正）。これは観察モード固有の問題ではなく、**レビュー・監視・改善・安全化の仕組みを追加する際に、効果検証可能性を設計していなかったこと**が本質である。

**原則:** リスク低減・品質向上・レビュー・自動判定・監視・最適化・改善のための仕組みを追加する場合は、実装前に以下を確認する。
...
3. 判断に必要なデータはどこに残るのか
4. 保存してはいけない情報は何か
5. いつ、何を見て、継続・調整・Gate化・廃止を判断するのか
```

特に「観察」「レビュー」「監視」「改善」「最適化」「Gate化検討」という言葉を使う仕組みには、後から判断できるデータ経路（ログ・JSONL・集計等）が必要になる。

**適用しなくてよいもの:** typo修正・文言修正・単純リファクタなど、効果を後から評価する必要がない変更には適用しない。過剰実装を避けるため、対象は上記の性質を持つ仕組み（レビュー・監視・改善・安全化・自動判定・最適化）に限定する。

**データ保存が不要な場合:** 保存しないと判断した場合は、代わりに「何をもって有効・完了と判断するのか」を設計時に明記する（例: 「テストが安定して通り続けることをもって完了とする」等）。データ経路が存在しないまま「観察中」「様子見」とだけ書いて放置しない。
