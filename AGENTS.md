...
| Meta Reviewer AI (Gemini) | `approved` または `changes_requested` |

GitHubのPRページでこれらのステータスを確認してからCEOに報告する。

### 結果ごとの行動（暫定）

| 結果 | 行動 |
|---|---|
| 両方 ✅ `approved` | CEOに報告してマージ待ち。次タスクには進まない |
| CI ✅ / Meta Review `changes_requested` | 指摘箇所を修正 → 同ブランチに追加コミット → 再度待機 |
| CI ❌ | エラーログを確認して修正 → 再度待機 |
| Meta Review `blocked` | **作業を即停止。CEOに報告。** 自己判断で修正しない |
